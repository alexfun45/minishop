...
export const OrdersManagement: React.FC = () => {
  // Предполагаем, что из хука можно достать метод deleteOrder. 
  // Если API его поддерживает, просто деструктуризируем его здесь
  ...
  const handleOrderDelete = async (orderId: number) => {
    const isConfirmed = window.confirm(`Вы уверены, что хотите безвозвратно удалить заказ #${orderId} из базы данных?`);
    ...
    try {
      if (typeof deleteOrder === 'function') {
        ...
        console.warn("Метод deleteOrder не реализован в вашем хуке useOrders");
        alert("Функция удаления еще не подключена к API бэкенда.");
      }
    } catch (error) {
        ...
        <div className="orders-manager">
          <div className="flex space-x-2 border-b border-gray-200 mb-6">
            ...
            <button
              ...
              className={`py-2 px-4 font-medium text-sm transition-all ${
                activeTab === 'cancelled' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              Отмененные
            </button>
                ...
                {filteredOrders.map((order) => (
                  <div key={order.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <div className="p-5 sm:p-6">
                            ...
                            <div>
                              ...
                              <div className="flex items-center mt-0.5">
                                ...
                                <span className={`ml-2 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ${
                                  order.payment_status === 'payment_success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                }`}>
                      ...
                      {/* НИЖНЯЯ ЧАСТЬ: Динамические кнопки управления */}
                      <div className="mt-6 flex flex-wrap items-center gap-3">
                        
                        {/* Показываем кнопки шагов кухни ТОЛЬКО если заказ НЕ отменен */}
                          ...
                          <>
                            ...
                            {order.status === 'ready' && (
                              <button
                                ...
                                className="flex-1 sm:flex-none px-6 py-2.5 bg-gray-800 text-white text-xs font-bold rounded-lg hover:bg-gray-900 shadow-sm transition-all"
                              >
                                Завершить и доставить
                              </button>
                            )}
                            
                            {/* Кнопка отмены активна для любого текущего живого статуса */}
                            <button
                        ...
                        {/* КНОПКА УДАЛЕНИЯ: Появляется только когда статус 'cancelled' */}
